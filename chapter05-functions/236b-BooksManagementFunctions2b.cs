...
public class Books2Functions2
{
    ...
    public static void Load()
    {
        ...
        {
            string[] data = File.ReadAllLines("books.dat");

            foreach (string s in data)
            {
                string[] parts = s.Split('|');
                b[count].title = parts[0];
                b[count].author = parts[1];
                b[count].year = Convert.ToUInt16(parts[2]);
                b[count].isElectronic = (parts[3] == "e");
                count++;
            }
        }
    }
    ...
    {
        string[] fileData = new string[count];
        for (int i = 0; i < count; i++)
        {
            fileData[i] = b[i].title + "|" + b[i].author
                + "|" + b[i].year
                + "|" + (b[i].isElectronic ? "e" : "p");
        }
        File.WriteAllLines("books.dat", fileData);
    }
