...
public class ComputerGames
{
    ...
    static void Save(string fileName, game[] g, int count)
    {
        IFormatter formatter = new BinaryFormatter();
        Stream outputFile = new FileStream(fileName,
            FileMode.Create, FileAccess.Write,
            FileShare.None);
        formatter.Serialize(outputFile, count);
        formatter.Serialize(outputFile, g);
        outputFile.Close();
    }

    static void Load(string fileName, ref game[] g, ref int count)
    {
        IFormatter formatter = new BinaryFormatter();
        Stream inputFile = new FileStream(fileName,
            FileMode.Open, FileAccess.Read,
            FileShare.Read);
        count = (int) formatter.Deserialize(inputFile);
    ...
    public static void Main()
    {
        ...
        string option;
        int currentGame = 0;
        if (File.Exists("games2.dat"))
        {
            ComputerGames.Load("games2.dat", ref g, ref currentGame);
        }
        ...
        ComputerGames.Save("games2.dat", g, currentGame);
    }
}
